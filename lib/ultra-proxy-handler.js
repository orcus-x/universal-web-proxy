  ...
  async handleRequest(req, res) {
    try {
      ...
      if (this.mightBeCloudflare(fullTargetUrl)) {
        try {
          console.log('Detected possible Cloudflare, trying HTTP/2 bypass...');
          ...
          console.log('HTTP/2 bypass failed:', error.message);
          lastError = error;
        }
      }
      ...
      if (!response) {
        try {
          ...
          console.log('Undici failed, trying Got...');
          lastError = error;
        }
      }
      ...
      if (!response) {
        try {
          ...
          console.log('Got failed, trying fallback...');
          lastError = error;
        }
      }
