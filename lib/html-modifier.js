        ...
        (function() {
          ...
          function rewriteUrl(url) {
            try {
              ...
              if (url.startsWith('javascript:') || url.startsWith('data:') || url.startsWith('mailto:') || url.startsWith('#')) {
                return url;
              }
              ...
              // Always rewrite URLs that point to the target domain
              if (absoluteUrl.origin === targetBase || absoluteUrl.href.includes(targetBase)) {
                return absoluteUrl.href.replace(targetBase, proxyHost);
              }

              // If it's an absolute URL to a different domain, still proxy it
              if (url.startsWith('http://') || url.startsWith('https://')) {
                // Replace the origin with proxy
                return absoluteUrl.href.replace(absoluteUrl.origin, proxyHost);
              }

              // For relative URLs, ensure they go through proxy
          ...
          Object.defineProperty(window, 'location', {
            ...
            set: function(value) {
              ...
              originalLocation.set.call(window, rewrittenUrl);
            },
            configurable: true
          });
